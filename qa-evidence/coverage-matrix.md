...
- **#size**: ホットバーアイテム名で `#size` が有効であることを確認（TC-HOTBAR-07）。`toComponent()` 経路でも ScaledSequence が適用されている可能性あり。
- **#glow**: ホットバーアイテム名ではテキスト描画は正常だが、発光エフェクト（LightMode）は FCS 経路のみ有効のため視覚差なし（CONDITIONAL）。
