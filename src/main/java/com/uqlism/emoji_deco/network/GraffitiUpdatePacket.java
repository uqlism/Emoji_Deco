...
public record GraffitiUpdatePacket(BlockPos pos, String[] lines, GraffitiAlignment alignment) {
    ...
    public static GraffitiUpdatePacket decode(FriendlyByteBuf buf) {
        ...
        return new GraffitiUpdatePacket(pos, lines, align);
    }
    ...
    public static void handle(GraffitiUpdatePacket pkt, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (be instanceof GraffitiBlockEntity g) {
                g.applyUpdate(pkt.lines, pkt.alignment);
            }
        });
